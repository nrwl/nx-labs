...
function getNpmLockfileDependencies(lockFileContent, lockFileHash, projectGraph) {
    const data = parsePackageLockFile(lockFileContent, lockFileHash);
...
function getDependencies(data, keyMap, projectGraph) {
    const dependencies = [];
    if (data.lockfileVersion > 1) {
        Object.entries(data.packages).forEach(([path, snapshot]) => {
                    ...
                    Object.entries(section).forEach(([name, versionRange]) => {
                        ...
                        if (target) {
                            const dep = {
                                source: sourceName,
                                target: target.name,
                                dependencyType: project_graph_1.DependencyType.static,
                            };
                            (0, project_graph_builder_1.validateDependency)(projectGraph, dep);
        ...
        Object.entries(data.dependencies).forEach(([packageName, snapshot]) => {
            addV1NodeDependencies(`node_modules/${packageName}`, snapshot, dependencies, keyMap, projectGraph);
        });
    }
    return dependencies;
}
...
function addV1NodeDependencies(path, snapshot, dependencies, keyMap, projectGraph) {
    ...
    if (snapshot.dependencies) {
        Object.entries(snapshot.dependencies).forEach(([depName, depSnapshot]) => {
            addV1NodeDependencies(`${path}/node_modules/${depName}`, depSnapshot, dependencies, keyMap, projectGraph);
        });
    }
