    ...
    if (verbose || !result.overallSuccess) {
      ...
      result.goalResults.forEach((goalResult, index) => {
        ...
        if (verbose && goalResult.output.length > 0) {
          devkit_1.logger.debug(
            `  Output: ${goalResult.output.slice(-5).join('\n  ')}`
          ); // Last 5 lines
        }
      });
    }
...
async function batchMavenExecutor(taskGraph, inputs) {
  ...
  const projectPromises = Array.from(tasksByProject.entries()).map(
    async ([projectName, projectTasks]) => {
      try {
        ...
        for (const { taskId, options } of projectTasks) {
          ...
          if (options.verbose) commonOptions.verbose = true;
          if (
            options.timeout &&
            options.timeout < (commonOptions.timeout || 300000)
          ) {
            commonOptions.timeout = options.timeout;
          }
        }
        // Execute all goals for this project in a single batch
        const batchResult = await executeMavenBatch(
          allGoals,
          commonOptions,
          process.cwd()
        );
