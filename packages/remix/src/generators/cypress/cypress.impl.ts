...
export default async function (
  ...
  const projSideEffects = cypressProjectGenerator(tree, {...options, standaloneConfig: true})

  const config = readProjectConfiguration(tree, options.name)
  tree.delete(joinPathFragments(config.sourceRoot, "support", "app.po.ts"));
  tree.write(joinPathFragments(config.sourceRoot, "integration", "app.spec.ts"), `describe('webapp', () => {
  beforeEach(() => cy.visit('/'));
