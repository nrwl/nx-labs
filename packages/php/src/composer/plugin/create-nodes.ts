...
function makeCreateNodesFromComposerJson(
  ...
  return async (
    ...
    if (
      ...
      minimatch(configFilePath, options.ignorePattern)
    )
      return {};
    ...
    // Must have a name to be a valid project.
    if (!composerJson.name) return {};

    const hash = await calculateHashForCreateNodes(
      projectRoot,
      normalizedOptions,
      context,
      maybeGetLockFiles(context.workspaceRoot)
    );
