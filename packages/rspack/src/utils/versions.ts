export const rspackCoreVersion = '~0.4.3';
export const rspackDevServerVersion = '~0.4.3';

export const rspackPluginMinifyVersion = '~0.4.3';
export const rspackPluginReactRefreshVersion = '~0.4.3';
export const lessLoaderVersion = '~11.1.3';

export const reactVersion = '~18.2.0';
export const reactDomVersion = '~18.2.0';
export const typesReactVersion = '~18.0.28';
export const typesReactDomVersion = '~18.0.10';

export const nestjsCommonVersion = '~9.0.0';
export const nestjsCoreVersion = '~9.0.0';
export const nestjsPlatformExpressVersion = '~9.0.0';
export const nestjsMicroservicesVersion = '~9.0.0';

export const lessVersion = '4.1.3';
export const sassVersion = '^1.42.1';
export const stylusVersion = '^0.55.0';

export const eslintPluginImportVersion = '2.27.5';
export const eslintPluginJsxA11yVersion = '6.7.1';
export const eslintPluginReactVersion = '7.32.2';
export const eslintPluginReactHooksVersion = '4.6.0';
